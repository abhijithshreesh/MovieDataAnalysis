...
class GenreTag(ActorTag):
    ...
    def combine_computed_weights(self, data_frame, model):
        """
        ...
        tag_df = self.get_model_weight(tf_weight_dict, idf_weight_dict, tag_df, model)
    ...
    def merge_genre_tag(self, genre, model):
        ...
        tag_dict = self.combine_computed_weights(genre_tag_frame[genre_tag_frame["genre"]==genre], model)
        print({genre: tag_dict})


if __name__ == "__main__":
    obj = GenreTag()
    obj.merge_genre_tag(genre="Children", model='TFIDF')
